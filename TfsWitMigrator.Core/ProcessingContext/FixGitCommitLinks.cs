...
namespace TfsWitMigrator.Core
{
    public class FixGitCommitLinks : ProcessingContextBase
    {
        ...
        public FixGitCommitLinks(MigrationEngine me) : base(me)
        {
        ...
        internal override void InternalExecute()
        {
            ...
            stopwatch.Start();
            //////////////////////////////////////////////////
            var gitRepoService = me.Target.Collection.GetService<GitRepositoryService>();
            var gitProjectRepoService = gitRepoService.QueryRepositories("*");
            ...
            foreach (WorkItem workitem in workitems)
            {
                ...
                foreach (Link l in workitem.Links)
                {
                    if ( l is ExternalLink && l.ArtifactLinkType.Name == "Fixed in Commit")
                    {
                        ...
                        string[] bits = Regex.Split( guidbits, "%2f");
                        string oldGitRepoId = bits[1];
                        string oldCommitId = bits[2];
                        var oldGitRepo = (from g in gitProjectRepoService where g.Id.ToString() == oldGitRepoId select g).SingleOrDefault();

                        if(oldGitRepo!=null && oldGitRepo.ProjectReference.Name != me.Target.Name)
                        {

                            var newGitRepo = (from g in gitProjectRepoService where g.Name == oldGitRepo.Name.Replace("Trayport.", "") && g.ProjectReference.Name != oldGitRepo.ProjectReference.Name select g).SingleOrDefault();
