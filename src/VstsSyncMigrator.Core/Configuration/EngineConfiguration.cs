...
namespace VstsSyncMigrator.Engine.Configuration
{
    public class EngineConfiguration
    {
        ...
        public static EngineConfiguration GetDefault()
        {
            ...
            ec.Processors.Add(new WorkItemMigrationConfig() { Enabled = false, ReplayRevisions= true, LinkMigration = true, AttachmentMigration = true, AttachmentWorkingPath = "c:\\temp\\WorkItemAttachmentWorkingFolder\\", UpdateCreatedBy =true, PrefixProjectToNodes = false, UpdateCreatedDate=true, UpdateSourceReflectedId=true, QueryBit = @"AND [TfsMigrationTool.ReflectedWorkItemId] = '' AND  [Microsoft.VSTS.Common.ClosedDate] = '' AND [System.WorkItemType] IN ('Shared Steps', 'Shared Parameter', 'Test Case', 'Requirement', 'Task', 'User Story', 'Bug')" });
