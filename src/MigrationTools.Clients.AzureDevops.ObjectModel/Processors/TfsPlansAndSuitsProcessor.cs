...
namespace MigrationTools.Processors
{
    ...
    /// </summary>
    public class TfsPlansAndSuitsProcessor : Processor
    {
        private TfsPlansAndSuitsProcessorOptions _Options;
        ...
        private ITestManagementService _sourceTestStore;
        private ITestManagementTeamProject _sourceTestProject;
        ...
        private ITestManagementService _targetTestStore;
        private ITestManagementTeamProject _targetTestProject;
        ...
        public TfsPlansAndSuitsProcessor(ProcessorEnricherContainer processorEnrichers,
        ...
        public override void Configure(IProcessorOptions options)
        {
            ...
            Log.LogInformation("TfsPlansAndSuitsProcessor::Configure");
            _Options = (TfsPlansAndSuitsProcessorOptions)options;
        }

        protected override void InternalExecute()
        {
            _sourceTestStore = Source.TfsCollection.GetService<ITestManagementService>();
            _sourceTestProject = _sourceTestStore.GetTeamProject(((TfsEndpointOptions)_Options.Source).Project);
            _sourceTestConfigs = _sourceTestProject.TestConfigurations.Query(_Options.TestConfigurationQuery);
            _sourceIdentityManagementService = Source.TfsCollection.GetService<IIdentityManagementService>();

            _targetTestStore = Target.TfsCollection.GetService<ITestManagementService>();
            _targetTestProject = _targetTestStore.GetTeamProject(((TfsEndpointOptions)_Options.Target).Project);
            _targetTestConfigs = _targetTestProject.TestConfigurations.Query(_Options.TestConfigurationQuery);
            _targetIdentityManagementService = Target.TfsCollection.GetService<IIdentityManagementService>();

            bool filterByCompleted = _Options.FilterCompleted;

            var stopwatch = Stopwatch.StartNew();
            var starttime = DateTime.Now;

            ITestPlanCollection sourcePlans = _sourceTestProject.TestPlans.Query(_Options.TestPlanQuery);

            List<ITestPlan> toProcess;
            if (filterByCompleted)
            {
                var targetPlanNames = (from ITestPlan tp in _targetTestProject.TestPlans.Query(_Options.TestPlanQuery) select tp.Name).ToList();
